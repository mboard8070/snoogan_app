...
from discord_bot import send_entry  # We'll use the pretty one for entries
...
class TradingStrategy:
    ...
    def _update_state(self):
        if self.position:
            ...
            os.remove(STATE_FILE)  # Clean up if position cleared
    ...
    def _attempt_entry(self, trend, chain):
        # DOUBLE PROTECTION: no entry if position in memory OR file exists
        ...
        if not sel.empty:
            ...
            long_strike = s + 1 if direction == "put" else s - 1  # proper vertical
            ...
            if c >= 0.20:
                ...
                # Pretty Discord alert via discord_bot
                send_entry(
    ...
    def _monitor_position(self, chain):
        ...
        if cur <= (p['entry_credit'] * 0.50):
            self._close_and_clear("TAKE PROFIT", cur)
        elif cur >= (p['entry_credit'] * 1.50):
            self._close_and_clear("STOP LOSS", cur)

    def _close_and_clear(self, reason, debit):
        from discord_bot import send_exit  # Lazy import to avoid circular

        pnl = round((self.position['entry_credit'] - debit) * 100, 2)
        self.daily_pnl += pnl

        send_exit(
            is_put=(self.position['side'] == "put"),
            short=self.position['short_strike'],
            long=self.position['long_strike'],
