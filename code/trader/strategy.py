...
class TradingStrategy:
    ...
    def _load_state(self):
        """
        Private method: Loads persistent state from strategy_state.json.
    ...
    def _save_state(self):
        """
        Private method: Persists current state to strategy_state.json.
    ...
    def run_cycle(self):
        """
        Main loop method called every minute.
    ...
    def _attempt_entry(self, ticker, trend, chain, price):
        ...
        if not candidates:
            print(f" [{ticker}] No valid delta spreads with credit >= $0.20 – skipping entry")
            return
        ...
        print(f" [ENTRY] {ticker} {'PUT' if is_put else 'CALL'} {short_strike:.1f}/{long_strike:.1f} | "
